...
export class MirrorSyncService {
  ...
  async pull(profile, remoteDir, localDir, dryRun = false) {
    ...
    // Try GNU find with printf first; fallback to standard find if unsupported
    let findCmd = `find . -maxdepth 5 -type f ! -path '*/.*' ! -path '*/node_modules/*' -printf '%P\\t%s\\t%T@\\n' 2>/dev/null || find . -maxdepth 5 -type f ! -path '*/.*' ! -path '*/node_modules/*'`;
