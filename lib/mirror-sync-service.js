...
export class MirrorSyncService {
  ...
  async pull(profile, remoteDir, localDir, dryRun = false) {
    ...
    for (const line of lines) {
      ...
      if (
        ...
        const localContent = fs.readFileSync(localFullPath);
      ...
      let remoteContent = '';
      try {
        remoteContent = await this.remoteFs.readFile(profile, remoteFullPath, null);
      } catch (err) {
      ...
      if (localHash !== remoteHash) {
        ...
        if (!dryRun) {
          ...
          fs.writeFileSync(localFullPath, remoteContent);
        }
      }
  ...
  async push(profile, localDir, remoteDir, force = false, dryRun = false) {
    ...
    for (const rel of localFiles) {
      ...
      const content = fs.readFileSync(localFull);
      ...
      if (!dryRun) {
        ...
        await this.remoteFs.writeFile(profile, remoteFull, content);
      }
      pushed.push(rel);
