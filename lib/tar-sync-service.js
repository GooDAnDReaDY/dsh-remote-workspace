...
export class TarSyncService {
  ...
  async pullTar(profile, remoteDir, localDir) {
    ...
    if (!remoteDir || typeof remoteDir !== 'string') throw new Error('remoteDir must be a non-empty string');
    fs.mkdirSync(localDir, { recursive: true });

    const client = await this.sshService.getConnection(profile);
    return new Promise((resolve, reject) => {
      const safeRemoteDir = shellQuote(remoteDir);
      const remoteCmd = `tar --exclude='.git' --exclude='node_modules' -czf - -C ${safeRemoteDir} .`;
  ...
  async pushTar(profile, localDir, remoteDir) {
    ...
    await this.sshService.exec(profile, `mkdir -p ${safeRemoteDir}`);

    return new Promise((resolve, reject) => {
      const remoteCmd = `tar -xzf - -C ${safeRemoteDir}`;
      client.exec(remoteCmd, (err, stream) => {
        if (err) return reject(err);
