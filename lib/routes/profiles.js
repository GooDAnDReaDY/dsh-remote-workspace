...
import { testProfileGroup } from '../profile-groups.js';
...
export function registerProfileRoutes(sctx, webServer, { store, sshService, alertService, tunnelService }) {
  ...
  sctx.effect(() => webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        const authPrompts = sshService.listKeyboardPrompts ? sshService.listKeyboardPrompts() : [];
        const sanitized = profiles.map((p) => (store.vault ? store.vault.sanitizeProfile(p) : maskProfile(p)));
        const terminalFontFamily = store.getTerminalFont ? store.getTerminalFont() : '';
        writeJson(res, 200, { ok: true, activeId, profiles: sanitized, tunnels, autoSync, alerts, authPrompts, terminalFontFamily });
      } catch (err) {
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        const content = typeof body.content === 'string' && body.content.trim()
          ? body.content
          : SshConfigParser.readDefaultConfig();
        if (!content) return writeJson(res, 400, { ok: false, error: 'SSH config content is required' });
        const report = SshConfigParser.parseReport(content, { baseDir: body.baseDir });
        for (const p of report.profiles) {
          await store.saveProfile(p);
        }
        writeJson(res, 200, {
          ok: true,
          count: report.profiles.length,
  ...
  sctx.effect(() => webServer.register({
    kind: 'exact',
    path: '/dsh-remote-workspace/auth/keyboard',
    handler: async (req, res) => {
      if (req.method !== 'POST') return writeJson(res, 405, { ok: false, error: 'POST only' });
      if (!isTrustedSettingsRequest(req)) return writeJson(res, 403, { ok: false, error: 'Forbidden' });
      try {
        const body = await readBody(req);
        const answers = Array.isArray(body.answers) ? body.answers.slice(0, 20).map((item) => String(item ?? '').slice(0, 256)) : [];
        const accepted = sshService.submitKeyboardPrompt(body.id || '', answers);
        if (!accepted) return writeJson(res, 404, { ok: false, error: 'No authentication prompt is waiting' });
        writeJson(res, 200, { ok: true });
      } catch (err) {
        writeJson(res, 500, { ok: false, error: err.message });
      }
  ...
  sctx.effect(() => webServer.register({
    kind: 'exact',
    path: '/dsh-remote-workspace/profiles/test-group',
    handler: async (req, res) => {
      if (req.method !== 'POST') return writeJson(res, 405, { ok: false, error: 'POST only' });
      if (!isTrustedSettingsRequest(req)) return writeJson(res, 403, { ok: false, error: 'Forbidden' });
      try {
        ...
        const selected = ids.map((id) => store.getProfile(id)).filter(Boolean);
        const results = await testProfileGroup(selected, (profile) => sshService.testConnection(profile));
  ...
  sctx.effect(() => webServer.register({
    kind: 'exact',
    path: '/dsh-remote-workspace/cluster',
    handler: async (req, res) => {
      if (req.method !== 'POST') return writeJson(res, 405, { ok: false, error: 'POST only' });
      if (!isTrustedSettingsRequest(req)) return writeJson(res, 403, { ok: false, error: 'Forbidden' });
      try {
        const body = await readBody(req);
        if (!body.command || typeof body.command !== 'string') return writeJson(res, 400, { ok: false, error: 'Command is required' });
        const selected = filterClusterProfiles(store.getProfiles(), {
          environment: body.environment,
          tags: Array.isArray(body.tags) ? body.tags : [],
          aliases: Array.isArray(body.aliases) ? body.aliases : []
        });
        const results = await runCluster(selected, body.command, {
