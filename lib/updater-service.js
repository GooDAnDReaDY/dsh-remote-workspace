...
/**
 * Host-side one-click updater for @goodandready/dsh-remote-workspace.
...
export function registerPluginUpdater(ctx, options) {
  ...
  const webServer = ctx?.get ? (ctx.get('webServer') ?? ctx?.webServer) : ctx?.webServer;
  if (!webServer || typeof webServer.register !== 'function') {
    ctx?.logger?.warn?.('[dsh-remote-workspace] webServer service unavailable, updater skipped')
    return () => {}
  }
  ...
  return webServer.register({
    ...
    handler: async (request, response) => {
      try {
        ...
        ctx?.logger?.warn?.(`[dsh-remote-workspace] plugin updater failed: ${String(error)}`)
