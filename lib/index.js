...
export const Config = z.object({
  profiles: z.array(z.object({
    ...
    jumpHosts: z.array(z.string()).default([]),
  ...
  terminalFontFamily: z.string().default('').description('CSS font family used by the terminal. Empty keeps the default monospace stack.')
});
...
export function apply(ctx, config = {}) {
  ...
  let currentConfig = {
    ...
    terminalFontFamily: typeof cfg.terminalFontFamily === 'string' ? cfg.terminalFontFamily : ''
  };

  const vaultService = new VaultService();
  const sshService = new SshService(ctx, vaultService);
  const idleTimer = setInterval(() => sshService.sweepIdle(), 60 * 1000);
  idleTimer.unref?.();
  ...
  const store = {
    ...
    getTerminalFont() { return currentConfig.terminalFontFamily || ''; },
    async setTerminalFont(fontFamily) {
      currentConfig.terminalFontFamily = normalizeTerminalFont(fontFamily);
      await persistConfig('terminal-font');
      return currentConfig.terminalFontFamily;
    },
  ...
  sshService.setProfileResolver((id) => store.getProfile(id));

  const safeProfilesService = {
    list() {
  ...
  ctx.provide('remoteProfiles');
  ctx.remoteProfiles = safeProfilesService;
