export function registerApiRoutes(ctx, sshService, remoteFs, mirrorSync, tunnelService, store, dockerService, healthService, watcherService) {
  ...
  ctx.inject(['webServer'], (sctx) => {
    ...
    // 7. POST /dsh-remote-workspace/file/view
    sctx.effect(() => webServer.register({
      kind: 'exact',
    ...
    // 8. POST /dsh-remote-workspace/file/save
    sctx.effect(() => webServer.register({
      kind: 'exact',
    ...
    // 9. POST /dsh-remote-workspace/terminal/create
    sctx.effect(() => webServer.register({
      kind: 'exact',
    ...
    // 10. POST /dsh-remote-workspace/terminal/input
    sctx.effect(() => webServer.register({
      kind: 'exact',
    ...
    // 11. GET /dsh-remote-workspace/terminal/stream (SSE)
    sctx.effect(() => webServer.register({
      kind: 'exact',
    ...
    // 12. POST /dsh-remote-workspace/terminal/close
    sctx.effect(() => webServer.register({
      kind: 'exact',
    ...
    // 13. POST /dsh-remote-workspace/sync
    sctx.effect(() => webServer.register({
      kind: 'exact',
      ...
      handler: async (req, res) => {
        ...
        if (!isTrustedSettingsRequest(req)) return writeJson(res, 403, { ok: false, error: 'Forbidden' });
        try {
          const { profileId, direction, force } = await readBody(req);
          const profile = (profileId ? store.getProfile(profileId) : null) || store.getActiveProfile();
          if (!profile) return writeJson(res, 404, { ok: false, error: 'Profile not found' });
    ...
    // 14. POST /dsh-remote-workspace/tunnels/start
    sctx.effect(() => webServer.register({
      kind: 'exact',
    ...
    // 15. POST /dsh-remote-workspace/tunnels/stop
    sctx.effect(() => webServer.register({
      kind: 'exact',
    ...
    // 16. GET /dsh-remote-workspace/tunnels/telemetry
    sctx.effect(() => webServer.register({
      kind: 'exact',
    ...
    // 17. GET /dsh-remote-workspace/health
    sctx.effect(() => webServer.register({
      kind: 'exact',
    ...
    // 18. GET /dsh-remote-workspace/docker/list
    sctx.effect(() => webServer.register({
      kind: 'exact',
    ...
    // 19. POST /dsh-remote-workspace/docker/action
    sctx.effect(() => webServer.register({
      kind: 'exact',
    ...
    // 20. POST /dsh-remote-workspace/docker/logs
    sctx.effect(() => webServer.register({
      kind: 'exact',
    ...
    // 21. POST /dsh-remote-workspace/watcher/toggle
    sctx.effect(() => webServer.register({
      kind: 'exact',
    ...
    // 22. POST /dsh-remote-workspace/transfer
    sctx.effect(() => webServer.register({
      kind: 'exact',
