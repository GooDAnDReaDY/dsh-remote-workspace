export function registerApiRoutes(ctx, sshService, remoteFs, mirrorSync, tunnelService, store) {
  ...
  // List configured profiles & status
  ctx.router.get('/api/remote-workspace/profiles', async (c) => {
    const profiles = store.getProfiles();
    return c.json({ profiles, activeId: store.getActiveId() });
  });

  // Save / Update profile
  ctx.router.post('/api/remote-workspace/profiles', async (c) => {
  ...
  // Test connection
  ctx.router.post('/api/remote-workspace/test', async (c) => {
  ...
  ctx.router.post('/api/remote-workspace/browse', async (c) => {
  ...
  // Active tunnels
  ctx.router.get('/api/remote-workspace/tunnels', (c) => {
