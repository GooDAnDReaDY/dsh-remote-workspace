...
- **Прокси и прыжки**: `proxyCommand` запускает команду в стиле OpenSSH. Токены: `%h`, `%p`, `%r`, `%n` и `%%`. `jumpHosts` — упорядоченный список идентификаторов профилей. Если массив пуст, используется `jumpHostId` через запятую. Первый бастион берётся из пула, следующие открывают отдельные соединения и закрываются вместе с целевой сессией.
- **Агент и одноразовый код**: `agentPath` задаёт сокет агента. Пустое значение использует `SSH_AUTH_SOCK`, а на Windows можно указать `pageant`. Запрос keyboard-interactive появляется на карточке и истекает через 60 секунд.
- **Простой пула**: соединение без туннеля и без выполняемой команды закрывается через 30 минут. Следующая команда открывает его снова.
- **Повтор до вывода**: если соединение оборвалось до первого вывода, команда повторяется до трёх раз. Таймаут команды, уже начатый вывод и `idempotent: false` не повторяются.
- **Отдельный терминал**: терминал не делит соединение пула. Закрытие терминала закрывает только его SSH-сессию.
...
- `remote_hosts`: Краткая markdown-таблица хостов без паролей, ключей и команд прокси. `query` обязателен; пустая строка показывает все хосты.
- `remote_cluster`: Одна команда на хостах, которые совпали по окружению, всем указанным меткам и необязательным псевдонимам. `maxWorkers` по умолчанию 8.
...
- **Индикатор статуса и задержки**: Проверка связи с отображением пинга и ОС удалённого узла. Бейдж шапки читает `/dsh-remote-workspace/state` и показывает проверку, отсутствие активного хоста, готовность или недоступность статуса.
- **Кнопки действий**: Быстрый запуск синхронизации и контроль туннелей прямо из настроек. Ошибка сохранения, удаления, выбора активного профиля, обзора каталога или остановки туннеля показывается в предупреждении на карточке. Проверка соединения отправляет поля профиля, включая `host`.
- **Подпись в списке плагинов**: английская `Remote Workspace` или китайская `远程开发工作区` из словарей карточки.
- **Версия в блоке обновления**: строка показывает установленную версию из ответа статуса. Пока ответ не пришёл, показывается нейтральная подпись неизвестной версии.
- **Группы хостов**: у профиля есть `environment`, `tags`, `location` и `description`. Карточка показывает плоский список, группы по окружению или по метке и может проверить группу целиком.
- **Файлы**: вкладка Files загружает и скачивает удалённый файл, показывает прогресс по байтам и даёт отменить передачу. Файл больше 512 МБ отклоняется.
- **Шрифт терминала**: `terminalFontFamily` — семейство шрифтов CSS. Пустое значение оставляет стандартный моноширинный набор.
- **Скрытая вкладка**: опрос состояния останавливается, пока вкладка браузера скрыта, и сразу обновляется, когда вкладка снова видна.
- **Боковая панель**: кнопка Remote слева открывает в центральной колонке хосты, терминал, файлы, контейнеры, туннели и кластер. Возврат в чат только скрывает колонку, буфер терминала сохраняется.
...
| `profile.agentPath` | `string` | `""` | Сокет SSH-агента. Пустое значение использует `SSH_AUTH_SOCK`. |
| `profile.proxyCommand` | `string` | `""` | OpenSSH ProxyCommand. Токены: `%h` `%p` `%r` `%n`. |
| `profile.jumpHosts` | `string[]` | `[]` | Идентификаторы бастионов, первый — вход. |
| `profile.jumpHostId` | `string` | `""` | Бастионы через запятую, если `jumpHosts` пуст. |
| `profile.environment` | `string` | `""` | Окружение для группы и фильтра кластера. |
| `profile.tags` | `string[]` | `[]` | Метки. Фильтр кластера требует совпадения каждой. |
| `profile.location` | `string` | `""` | Подпись места. |
| `profile.description` | `string` | `""` | Заметка. |
| `terminalFontFamily` | `string` | `""` | Семейство шрифта терминала. Пусто — стандартный моноширинный набор. |
...
### `remote_hosts`
Список хостов для модели без секретов.
- **Параметры**:
  - `query` (`string`, обязательно): Поиск по имени, хосту или идентификатору. Пустая строка показывает все.
- **Возвращает**: Markdown-таблицу. Пароль, ключ, путь к ключу, passphrase, сокет агента и proxy command не включаются.

### `remote_cluster`
Одна команда на отфильтрованном наборе хостов.
- **Параметры**:
  - `command` (`string`, обязательно): Команда оболочки.
  - `environment` (`string`): Точное имя окружения.
  - `tags` (`string`): Метки через запятую. Нужны все.
  - `aliases` (`string`): Идентификаторы или имена через запятую.
  - `maxWorkers` (`number`): Параллельность, по умолчанию 8.
- **Возвращает**: Строку на каждый хост: успех, код выхода, длительность, stdout, stderr и ошибка.
- **Ограничение**: Сбой соединения одного хоста не останавливает остальные.
...
| `POST` | `/dsh-remote-workspace/profiles/import-ssh-config` | Импорт `~/.ssh/config` или переданного текста. | `{ content?: string }` |
| `POST` | `/dsh-remote-workspace/profiles/test-group` | Проверка сохранённых профилей по списку id. | `{ ids: string[] }` |
| `POST` | `/dsh-remote-workspace/cluster` | Команда на отфильтрованных сохранённых профилях. | `{ command, environment?, tags?, aliases?, maxWorkers? }` |
| `GET` | `/dsh-remote-workspace/file/download` | Поток удалённого файла. Query: `profileId`, `filePath`. | — |
| `POST` | `/dsh-remote-workspace/file/upload` | Загрузка тела файла. Query: `profileId`, `filePath`. | байты файла |
| `POST` | `/dsh-remote-workspace/terminal/font` | Сохранение шрифта терминала. | `{ fontFamily: string }` |
| `POST` | `/dsh-remote-workspace/auth/keyboard` | Ответ keyboard-interactive. | `{ id, answers }` |
